...
- 強度が変わる場合は必ずセッションを分ける。「目的×強度×理由」を明確にすること。
- 各セッションごとに距離を明確にし、小計を出せる構造にすること。

【Pre-Main と Main の強度差（必須）】
Pre-Main は Main を最大化するための橋渡し。Main より一段階抑える。Main④→Pre-Main③、Main⑤→Pre-Main④、Main⑥→Pre-Main④〜⑤。Pre-Main で Main と同じ強度にしない。
...
function buildConditionInstructions(
  ...
  const periodGuide: Record<string, string> = {
    '1': `①リカバリー期: 回復・感覚維持。③は短時間アクセントのみ。内容軽め、他期より負荷を下げる。Diveなし。${targetDist ? `目標距離: 約${targetDist}m` : ''}`,
    '2': `②基礎形成期: EN1〜EN2中心に安定。レース色を抑え、Mainは段階的強度設計。Diveなし。耐乳酸MAX禁止。${targetDist ? `目標距離: 約${targetDist}m` : ''}`,
    '3': `③発展形成期: 質と量の両立。④以上の割合をやや意識。Kick・Pullは2構成必須。Diveなし。${targetDist ? `目標距離: 約${targetDist}m` : ''}`,
    '4': `④スピード持久期: ⑤の本数管理を明確に。Pre-Mainは④までで抑えMain最大化。Diveなし。耐乳酸MAX禁止。${targetDist ? `目標距離: 約${targetDist}m` : ''}`,
    '5': `⑤耐乳酸期: 全体5000m前後を確保。⑥は短距離・低本数で管理。量の中で耐える力を養う。${targetDist ? `目標距離: 約${targetDist}m` : ''}`,
    '6': `⑥調整期: 距離と強度を明確に下げる。段階的疲労除去。Dive導入可。${targetDist ? `目標距離: 約${targetDist}m` : ''}`,
    '7': `⑦テーパー期: 量より質を優先。疲労を抜きキレを出す。距離配分を他期より軽く。${targetDist ? `目標距離: 約${targetDist}m` : ''}`,
  };
