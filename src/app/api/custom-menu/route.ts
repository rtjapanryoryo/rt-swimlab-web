...
【総距離とブロック合計の一致（必須）】
各ブロックの距離×本数×セットを合計し、total（合計距離）にその値を記載すること。ブロック合計と total が不一致になることは禁止。設計後に必ず検算すること。

【Pre-Main と Main の強度差（必須）】
Pre-Main は Main より一段階抑えた橋渡し役。Main が④なら Pre-Main は③、Main が⑤なら Pre-Main は④、Main が⑥なら Pre-Main は④〜⑤。Pre-Main で Main と同じ強度にしない。

【ブロックの複数構成（推奨）】
- W-up: 2〜3段階に分けた構成（例: 200m A1 → 200m SKPS → 100m Build）
- Kick: 発展形成期・スピード持久期・耐乳酸期では2構成
- Pull: Fr中心で2構成、質の違いを持たせる
- Main: 基礎形成期以降は Pre-Main を含めて3段階程度

【耐乳酸期の特別ルール】
- 120分練習では全体で約5000m前後を目安。Main 以外のブロック（W-up/Kick/Pull/Pre-Main）でも距離を積む。
- Main の割合管理: ⑥の総距離目安を設定し、④以上の割合を明確にする。`;
...
function buildConditionInstructions(
  ...
  const periodGuide: Record<string, string> = {
    ...
    '5': `⑤強化期 (耐乳酸): 乳酸耐性・レースペース持続。メイン⑥・非メイン④まで。耐乳酸MAXはこの期のみ。強度とボリュームの両立が重要。120分では約5000m前後を目安に、Main以外のブロック（W-up/Kick/Pull/Pre-Main）でも距離を積む。${targetDist ? `目標距離: 約${targetDist}m（ユーザー指定）` : ''}`,
...
【expectedEffect】このメニューで得られる効果を2〜3行で。
{
  ...
  "warmUp": "2〜3段階構成。種目はCho固定。例: Cho 200m（A1）→ Cho 200m SKPS（A1）→ Cho 100m Build（EN1）",
  "drill": "ドリル名 本数×距離m（内容）。複数可。例: 片手ドリル 6×50m（左右交互）",
  "kick": "Kick 2構成推奨（発展・スピード持久・耐乳酸期）。例: Kick 4×50m（Des）（EN1）+ Kick 4×50m（Fins）（EN2）",
  "pull": "Pull 2構成。Fr中心で質の違いを。例: Pull Fr 4×50m（DPS）（EN1）+ Pull Fr 4×100m（EN2）",
  "preMain": "Pre-Main 本数×距離m（強度）。Mainより一段階下。例: Pre-Main 3×50m（EN2）",
