...
export default function CustomersPage() {
  ...
  // 登録番号マップ（created_at昇順で1から付番）
  const regNoMap = useMemo(() => {
    const sorted = [...regularCustomers].sort((a, b) => a.created_at.localeCompare(b.created_at));
  ...
  const filtered = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      ...
      if (sortKey === 'reg_no') {
        av = regNoMap.get(a.id) ?? 0;
        bv = regNoMap.get(b.id) ?? 0;
      } else {
  ...
  const handleSort = (key: SortKey) => {
    ...
    else { setSortKey(key); setSortAsc(key === 'reg_no'); }
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-amber-50 rounded-xl border border-amber-200 p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-xs font-bold text-amber-700 uppercase tracking-wider">管理者アカウント（{adminAccounts.length}件）</p>
        </div>
        {adminAccounts.length === 0 ? (
          <p className="text-xs text-amber-600">管理者アカウントなし</p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {adminAccounts.map(a => (
              <div key={a.id} className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-lg border border-amber-200 shadow-sm">
                <div className="w-6 h-6 rounded-full bg-amber-100 flex items-center justify-center shrink-0">
                  <span className="text-[10px] font-bold text-amber-600">{(a.display_name ?? '?').charAt(0)}</span>
                </div>
                <div>
                  <p className="text-xs font-semibold text-slate-800">{a.display_name ?? '（未設定）'}</p>
                  <p className="text-[10px] text-slate-400">生成 {a.total_usage_count} 回 · 登録 {fmtDateShort(a.created_at)}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-2 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-sky-400 w-48"
          />
        </div>
        ...
        <select
          ...
          className="px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-sky-400"
        >
          <option value="reg_no">登録番号順</option>
          <option value="created_at">登録日順</option>
          <option value="total_usage_count">累計利用数順</option>
          <option value="total_spend">累計収益順</option>
          <option value="freq_per_week">週間頻度順</option>
          <option value="last_active">最終利用日順</option>
        </select>

        {/* 昇順/降順 */}
        <button
          onClick={() => setSortAsc(p => !p)}
          className="flex items-center gap-1.5 px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white hover:bg-slate-50 transition-colors font-medium text-slate-600"
        >
          {sortAsc ? (
            <><svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg>昇順</>
          ) : (
          ...
          <table className="min-w-full divide-y divide-slate-100">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map(c => {
                ...
                const regNo    = regNoMap.get(c.id) ?? '—';
                return (
                  <tr key={c.id} className="hover:bg-slate-50/60 transition-colors">
                    {/* # 登録番号 */}
                    <td className="px-4 py-3 text-xs font-mono text-slate-400 tabular-nums w-10">
                      {regNo}
                    </td>
                    {/* 顧客名 */}
