  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">試合日を基準にAIが期を自動配分します（後から変更可能）</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">基本情報</p>

          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">計画名 *</label>
        ...
        {/* 目標試合 */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">目標試合</p>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">試合日 *</label>
              <input type="date" value={form.goal_meet_date} onChange={set('goal_meet_date')} required
                min={form.start_date}
                className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">試合名</label>
              <input type="text" value={form.goal_meet_name} onChange={set('goal_meet_name')}
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select value={form.goal_event} onChange={set('goal_event')}
                className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400">
                {EVENTS.map(e => <option key={e} value={e}>{e}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">距離タイプ</label>
              <select value={form.goal_distance_type} onChange={set('goal_distance_type')}
                className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400">
                <option value="S">S（短距離）</option>
                <option value="M">M（中距離）</option>
                <option value="D">D（長距離）</option>
              </select>
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">目標タイム</label>
              <div className="flex items-center gap-1">
                <input type="number" value={form.goal_time_mm} onChange={set('goal_time_mm')}
                ...
                <input type="number" value={form.goal_time_ss} onChange={set('goal_time_ss')}
        ...
        <div className="bg-amber-50 rounded-2xl border border-amber-200 p-5 space-y-3">
          <div>
            <p className="text-xs font-bold text-amber-700 uppercase tracking-wider">コーチ指示・AI制約</p>
            <p className="text-xs text-amber-600 mt-0.5">ここに入力した内容はAIのすべての提案・生成に最優先で反映されます</p>
          </div>
          <textarea
              ...
              {preview.map(a => {
                ...
                return (
                  <div key={a.period} className={`flex items-center gap-3 px-3 py-2 rounded-xl ${meta.color}`}>
                    <span className={`text-xs font-bold px-2 py-0.5 bg-white/60 rounded-lg ${meta.textColor}`}>
                      期{a.period} {meta.short}
                    </span>
                    <span className="text-xs text-slate-600">
                      {fmtDate(a.start_date)} 〜 {fmtDate(a.end_date)}（{a.weeks}週間）
                    </span>
                    <span className="ml-auto text-xs text-slate-400">
                      週{(a.target_weekly_volume_m / 1000).toFixed(0)}km目標
                    </span>
                  </div>
                );
              })}
