...
export default function MyPageMenus() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-bold text-slate-900">メニューログ</h1>
        <p className="text-slate-600 mt-1">過去に作成した練習メニューを日時付きで確認できます。</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="p-8 bg-white rounded-xl shadow-sm border border-slate-200 text-center text-slate-600">
          保存されたメニューはありません。トップでメニューを作成し、「保存」ボタンでログに追加できます。
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-2">
            <h2 className="text-sm font-semibold text-slate-700">作成ログ一覧</h2>
              ...
              {menus.map((m) => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg border transition-colors ${
                    selectedId === m.id ? 'border-slate-400 bg-slate-100' : 'border-slate-200 bg-white hover:bg-slate-50'
                  }`}
                >
            ...
            {selected ? (
              <div className="p-4 bg-white rounded-xl shadow-sm border border-slate-200 max-h-[500px] overflow-y-auto">
                ...
                {selected.result.rawText ? (
                  <pre className="whitespace-pre-wrap font-sans text-sm text-gray-800">{selected.result.rawText}</pre>
                ) : (
                  <MenuSheet
