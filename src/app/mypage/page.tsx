...
const menuItems: Array<{
  ...
  { href: '/mypage/menu', label: 'RT swim lab', description: '練習メニューをクイック作成・カスタム作成で生成', emoji: '🏊', gradient: 'from-cyan-400 to-teal-500' },
  { href: '/mypage/genetic', label: 'RT GENE PROFILE', description: '遺伝子検査結果PDFを確認・アップロード', emoji: '🧬', gradient: 'from-violet-400 to-purple-500' },
  { href: COMMUNITY_URL || '#', label: 'RTコミュニティ', description: 'RT公式コミュニティに参加', emoji: '💬', gradient: 'from-orange-400 to-amber-500', external: true, disabled: !COMMUNITY_URL },
  { href: '/mypage/settings', label: 'アカウント情報', description: '表示名・パスワード・有料プラン・アカウント連携を変更', emoji: '⚙️', gradient: 'from-sky-400 to-cyan-500' },
];
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {menuItems.map((item) => {
              ...
              const content = (
                <>
                  <span className={`inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-br ${item.gradient} text-2xl shadow-lg mb-4`}>
                    {item.emoji}
                  </span>
              ...
              if ('disabled' in item && item.disabled) {
                return (
                  <div key={item.label} className={disabledClass}>
                    <span className={`inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-slate-200 text-2xl mb-4`}>
                      {item.emoji}
                    </span>
